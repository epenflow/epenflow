...
const Maximize = React.forwardRef<SVGSVGElement, IconProps>(
	({ className, ...rest }, ref) => {
		...
		const id = `maximize-icon-${randomId}`;

		return (
			<svg {...{ ...rest, ref, id, className: cn(className) }}>
...
const Minimize = React.forwardRef<SVGSVGElement, IconProps>(
	({ className, ...rest }, ref) => {
		const randomId = React.useId();
		const id = `minimize-icon-${randomId}`;
...
export { Maximize, Minimize, Close };
